...
import com.cloudwebrtc.webrtc.record.MixedAudioSamplesInterceptor;
import com.cloudwebrtc.webrtc.record.OutputAudioSamplesInterceptor;
import com.cloudwebrtc.webrtc.record.RecorderAudioInterceptor;
...
public class GetUserMediaImpl {
    ...
    private MixedAudioSamplesInterceptor mixedSamplesInterceptor = null;
    private PlaybackSamplesReadyCallbackAdapter playbackSamplesReadyCallbackAdapter = null;
    JavaAudioDeviceModule audioDeviceModule;
    private final SparseArray<MediaRecorderImpl> mediaRecorders = new SparseArray<>();
    private final Map<Integer, FallbackAudioRecorder> fallbackAudioRecorders = new HashMap<>();
    ...
    void setPlaybackSamplesReadyCallbackAdapter(
    ...
    void startRecordingToFile(
        ...
        startRecordingToFile(path, id, videoTrack, audioChannels, false);
    }

    void startRecordingToFile(
            String path, Integer id, @Nullable VideoTrack videoTrack, @Nullable List<AudioChannel> audioChannels, boolean useFallbackAudio)
            throws Exception {
        RecorderAudioInterceptor interceptor = null;
        boolean includeInput = audioChannels != null && audioChannels.contains(AudioChannel.INPUT);
        boolean includeOutput = audioChannels != null && audioChannels.contains(AudioChannel.OUTPUT);

        if (includeInput && includeOutput) {
            if (playbackSamplesReadyCallbackAdapter == null) {
                throw new Exception("Playback audio samples are not available");
            }
            if (mixedSamplesInterceptor == null) {
                mixedSamplesInterceptor = new MixedAudioSamplesInterceptor(
                        inputSamplesInterceptor,
                        playbackSamplesReadyCallbackAdapter);
            }
            interceptor = mixedSamplesInterceptor;
        ...
        if (useFallbackAudio && includeInput) {
            final FallbackAudioRecorder fallback = new FallbackAudioRecorder(inputSamplesInterceptor);
            fallbackAudioRecorders.put(id, fallback);
            fallback.start();
        }
    }
    ...
    void stopRecording(Integer id, String albumName) {
        try {
            FallbackAudioRecorder fallback = fallbackAudioRecorders.remove(id);
            if (fallback != null) {
                fallback.stop();
            }
            inputSamplesInterceptor.setNativeSampleListener(null);
            ...
            if (mediaRecorder != null) {
                ...
                if (file != null && albumName != null && !albumName.isEmpty()) {
    ...
    private static class FallbackAudioRecorder {
        private final AudioSamplesInterceptor interceptor;
        ...
        public FallbackAudioRecorder(AudioSamplesInterceptor interceptor) {
        ...
        public synchronized void start() {
            ...
            if (minBufferSize == AudioRecord.ERROR || minBufferSize == AudioRecord.ERROR_BAD_VALUE) {
                Log.e(TAG, "Invalid buffer size for FallbackAudioRecorder");
                return;
            }

            int bufferSize = Math.max(minBufferSize, 960 * 4);
            ...
            try {
                ...
                Log.e(TAG, "Failed to instantiate AudioRecord: " + e.getMessage());
                return;
            }
            ...
            thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    byte[] buffer = new byte[960]; // 10ms at 48kHz mono
                    while (isRunning) {
                        int bytesRead = audioRecord.read(buffer, 0, buffer.length);
                        if (bytesRead > 0 && isRunning) {
                            byte[] data = new byte[bytesRead];
                            System.arraycopy(buffer, 0, data, 0, bytesRead);
                            interceptor.onFallbackAudioRecordSamplesReady(
                                new org.webrtc.audio.JavaAudioDeviceModule.AudioSamples(
            ...
            }, "FallbackAudioRecorderThread");
            thread.start();
            Log.d(TAG, "FallbackAudioRecorder started");
        }
        ...
        public synchronized void stop() {
            ...
            Log.d(TAG, "FallbackAudioRecorder stopped");
        }
    }
}

...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  @Nullable ConstraintsMap androidAudioConfiguration) {
    ...
    getUserMediaImpl.setPlaybackSamplesReadyCallbackAdapter(playbackSamplesReadyCallbackAdapter);
    ...
    switch (call.method) {
      ...
      case "startRecordToFile":
        ...
        try {
          ...
          Object audioChannelsArg = call.argument("audioChannels");
          if (audioChannelsArg instanceof List) {
            for (Object value : (List<?>) audioChannelsArg) {
              if (value instanceof Number) {
                int channelIndex = ((Number) value).intValue();
                if (channelIndex >= 0 && channelIndex < AudioChannel.values().length) {
                  audioChannels.add(AudioChannel.values()[channelIndex]);
                }
              }
            }
          ...
          Boolean useFallbackAudio = call.argument("useFallbackAudio");
          if (useFallbackAudio == null) {
              useFallbackAudio = false;
          }
          if (videoTrack != null || !audioChannels.isEmpty()) {
            getUserMediaImpl.startRecordingToFile(path, recorderId, videoTrack, audioChannels, useFallbackAudio);
            result.success(null);
          } else {
      ...
      case "stopRecordToFile":
        ...
        new Thread(() -> {
          try {
            ...
            resultError("stopRecordToFile", e.getMessage(), result);
          }
        }, "FlutterWebRTCStopRecorder").start();
        break;
      case "captureFrame": {
        String path = call.argument("path");

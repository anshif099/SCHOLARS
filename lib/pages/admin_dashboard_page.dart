...
import '../services/nakshatra_service.dart';
...
class _AdminSettingsTabState extends State<_AdminSettingsTab> {
        ...
        : SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Divider(),
                const SizedBox(height: 30),
                _buildPreloadedNakshatrasSection(),
  ...
  Widget _buildPreloadedNakshatrasSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            Text(
              'Pre-loaded Nakshatras Settings',
              style: GoogleFonts.poppins(
                fontSize: 18,
                fontWeight: FontWeight.bold,
                color: AppColors.primaryNavy,
        ...
        const SizedBox(height: 6),
        Text(
          'Manual adding of Nakshatras has been removed. All 27 traditional Nakshatras are automatically preloaded into the system.',
          style: GoogleFonts.poppins(
            fontSize: 13,
            color: AppColors.textLight,
          ),
        ...
        GridView.builder(
          ...
          itemCount: NakshatraService.preloadedNakshatras.length,
          itemBuilder: (context, index) {
            final nakshatra = NakshatraService.preloadedNakshatras[index];
            return Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    '${nakshatra.id}. ${nakshatra.englishName}',
                    style: GoogleFonts.poppins(
                      fontSize: 13,
                      fontWeight: FontWeight.bold,
                      color: AppColors.textPrimary,
...
// ─────────────────────────────────────────────
//  Nakshatra Counter Booking Tab Widget
// ─────────────────────────────────────────────

class _CounterBookingTab extends StatefulWidget {
  const _CounterBookingTab();

  @override
  State<_CounterBookingTab> createState() => _CounterBookingTabState();
}

class _CounterBookingTabState extends State<_CounterBookingTab> {
  final _formKey = GlobalKey<FormState>();
  final _devoteeNameController = TextEditingController();
  final _mobileController = TextEditingController();
  final _poojaRateController = TextEditingController(text: '100');

  String _selectedPooja = 'Pushpanjali';
  Nakshatra _selectedNakshatra = NakshatraService.preloadedNakshatras.first;
  int _repeatMonths = 6; // Options: 1, 3, 6, 12 months (up to 1 year)
  DateTime _startDate = DateTime.now();

  List<NakshatraBookingDate> _calculatedDates = [];
  bool _isSaving = false;

  final List<String> _poojaOptions = [
    'Pushpanjali',
    'Archana',
    'Payasam',
    'Ganapathi Homam',
  ...
  late final DatabaseReference _bookingsRef =
      FirebaseDatabase.instance.ref().child('nakshatra_bookings');
  late final Stream<DatabaseEvent> _bookingsStream =
      _bookingsRef.onValue.asBroadcastStream();
  ...
  void dispose() {
    ...
    _poojaRateController.dispose();
    super.dispose();
  }

  void _recalculateDates() {
    setState(() {
      _calculatedDates = NakshatraService.calculateNakshatraDates(
        nakshatra: _selectedNakshatra,
        startDate: _startDate,
        monthsCount: _repeatMonths,
      );
    });
  }

  int get _selectedDatesCount =>
      _calculatedDates.where((d) => d.isSelected).length;

  double get _totalAmount {
    final rate = double.tryParse(_poojaRateController.text.trim()) ?? 0;
    return rate * _selectedDatesCount;
  }

  Future<void> _submitBooking() async {
    if (!_formKey.currentState!.validate()) return;

    final selectedDates = _calculatedDates.where((d) => d.isSelected).toList();
    if (selectedDates.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select at least one repeat date for booking.'),
          backgroundColor: AppColors.accentRed,
          behavior: SnackBarBehavior.floating,
        ),
      );
    ...
    try {
      ...
      final dateList = selectedDates.map((d) {
        final dateStr =
      ...
      await newBookingRef.set({
        ...
        'devotee_name': _devoteeNameController.text.trim(),
        'mobile': _mobileController.text.trim(),
        'pooja_name': _selectedPooja,
        'pooja_rate': double.tryParse(_poojaRateController.text.trim()) ?? 0,
        'nakshatra_id': _selectedNakshatra.id,
        'nakshatra_name': _selectedNakshatra.englishName,
        'nakshatra_malayalam': _selectedNakshatra.malayalamName,
        'repeat_months': _repeatMonths,
        'selected_dates': dateList,
        'total_dates': selectedDates.length,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Nakshatra repeat booking submitted! (${selectedDates.length} dates booked across $_repeatMonths months)',
            ),
            backgroundColor: const Color(0xFF10B981),
            behavior: SnackBarBehavior.floating,
          ),
  ...
  void _showReceiptDialog(Map<String, dynamic> booking) {
    final dates = (booking['selected_dates'] as List?) ?? [];
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  child: Column(
                    children: [
                      _receiptRow('Devotee Name', booking['devotee_name'] ?? 'N/A'),
                      _receiptRow('Mobile', booking['mobile'] ?? 'N/A'),
                      _receiptRow('Pooja / Vazhipadu', booking['pooja_name'] ?? 'N/A'),
                      _receiptRow(
                        'Nakshatra',
                        '${booking['nakshatra_name']} (${booking['nakshatra_malayalam']})',
                      ),
                      _receiptRow('Duration', '${booking['repeat_months']} Months Repeat'),
                      _receiptRow('Total Dates Booked', '${booking['total_dates']} Days'),
                      _receiptRow('Total Amount', '₹ ${booking['total_amount']}'),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                ...
                ListView.separated(
                  ...
                  itemBuilder: (context, idx) {
                    final d = Map<String, dynamic>.from(dates[idx] as Map);
                    return Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                          '${idx + 1}. ${d['date']} (${d['day_of_week']})',
          ...
          Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    'Nakshatra Counter Repeat Booking',
                    style: GoogleFonts.poppins(
                      fontSize: 20,
                      fontWeight: FontWeight.bold,
                      color: AppColors.primaryNavy,
                  ...
                  Text(
                    'Book pooja for single date or repeat for 3, 6, or 12 months (1 year)',
                    style: GoogleFonts.poppins(
                      fontSize: 12,
                      color: AppColors.textLight,
                    ),
          ...
          Form(
            ...
            child: Column(
              ...
              children: [
                ...
                // Booking & Nakshatra Selection Card
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
                    color: AppColors.cardBackground,
                        ...
                        children: [
                          Expanded(
                            child: DropdownButtonFormField<String>(
                              ...
                              decoration: InputDecoration(
                                labelText: 'Select Pooja / Vazhipadu',
                                prefixIcon: const Icon(Icons.local_florist_outlined, size: 18),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(12),
                                ),
                              ),
                              items: _poojaOptions.map((p) {
                                return DropdownMenuItem(value: p, child: Text(p));
                              }).toList(),
                              onChanged: (val) {
                                if (val != null) setState(() => _selectedPooja = val);
                              },
                            ),
                          ),
                          const SizedBox(width: 14),
                          Expanded(
                            child: TextFormField(
                              controller: _poojaRateController,
                              keyboardType: TextInputType.number,
                              style: GoogleFonts.poppins(fontSize: 14),
                              decoration: InputDecoration(
                                labelText: 'Rate per Pooja (₹)',
                        ...
                        children: [
                          ...
                          Expanded(
                            child: DropdownButtonFormField<Nakshatra>(
                              ...
                              decoration: InputDecoration(
                                labelText: 'Select Janma Nakshatra (Preloaded 27 Stars)',
                                prefixIcon: const Icon(Icons.star_outline_rounded, size: 18),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(12),
                                ),
                              ),
                              items: NakshatraService.preloadedNakshatras.map((n) {
                              ...
                              children: [
                                ...
                                Row(
                                  children: [1, 3, 6, 12].map((m) {
                                    final isSelected = _repeatMonths == m;
                                    final label = m == 1
                                        ? '1 Mo'
                                        : m == 12
                                            ? '1 Yr (12M)'
                ...
                // Calculated Repeat Dates Preview Section
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
                    color: AppColors.cardBackground,
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Text(
                                      '$_selectedDatesCount / ${_calculatedDates.length} Selected',
                                      style: GoogleFonts.poppins(
                                        fontSize: 12,
                                        fontWeight: FontWeight.bold,
                                        color: AppColors.primaryNavy,
                              ...
                              Text(
                                'Prokerala Calendar Panchang Shift: Nakshatra day calculated for each month across $_repeatMonths months',
                                style: GoogleFonts.poppins(
                                  fontSize: 12,
                                  color: AppColors.textLight,
                                ),
                      ...
                      GridView.builder(
                        ...
                        itemCount: _calculatedDates.length,
                        itemBuilder: (context, index) {
                          final item = _calculatedDates[index];
                          final dateFormatted =
                              '${item.date.day} ${item.monthLabel}';
                          return InkWell(
                            onTap: () {
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          '${item.dayOfWeek} • ${_selectedNakshatra.malayalamName}',
                                          style: GoogleFonts.poppins(
                                            fontSize: 11,
                                            color: AppColors.textLight,
                                          ),
                ...
                // Total Summary & Submit Action Card
                Container(
                  padding: const EdgeInsets.all(20),
                  decoration: BoxDecoration(
                    color: AppColors.primaryNavy,
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          ...
                          Text(
                            '$_selectedDatesCount Dates × ₹${_poojaRateController.text} (${_selectedNakshatra.englishName})',
                            style: GoogleFonts.poppins(
                              fontSize: 12,
                      ...
                      ElevatedButton.icon(
                        ...
                        icon: _isSaving
                            ...
                            : const Icon(Icons.check_circle_rounded),
                        label: Text(
                          _isSaving ? 'Saving...' : 'Book Now & Save',
                          style: GoogleFonts.poppins(
                            fontWeight: FontWeight.bold,
                            fontSize: 15,
                          ),
                ...
                // Recent Counter Bookings Section
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (!snapshot.hasData || snapshot.data!.snapshot.value == null) {
                      return Container(
                        ...
                        child: Text(
                          'No Nakshatra counter bookings created yet.',
                          style: GoogleFonts.poppins(color: AppColors.textLight),
                        ),
                      );
                    }
                    ...
                    list.sort((a, b) => (b['created_at'] as int? ?? 0)
                        .compareTo(a['created_at'] as int? ?? 0));
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      'Pooja: ${item['pooja_name']} • Star: ${item['nakshatra_name']} (${item['nakshatra_malayalam']})',
                                      style: GoogleFonts.poppins(
                                        fontSize: 12,
                                        color: AppColors.primaryNavy,
                                        fontWeight: FontWeight.w500,
                                    ...
                                    Row(
                                      children: [
                                        Container(
                                          ...
                                          child: Text(
                                            '${item['repeat_months']} Months Repeat',
                                            style: GoogleFonts.poppins(
                                              fontSize: 11,
                                              fontWeight: FontWeight.bold,
                                              color: const Color(0xFF10B981),
